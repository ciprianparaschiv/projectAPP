...
*/
				...
				if(!isset($hoursss[$row['timing_project']])) { $hours_harvest[$row['timing_project']]=0; $hoursss[$row['timing_project']]=0;  $users_hour[$row5['timing_project']][$row5['user_id']]=''; }
					...
					$users_hour[$row5['timing_project']][$row5['user_id']]=$row5['user_name'];
					 ...
					 if($row5['timing_end']<=strtotime('11-04-2022'))   {   $hours_harvest[$row5['timing_project']]+=(($row5['timing_end']-$row5['timing_start'])+1)*1.1;     } else
					 	{
					    $hours_harvest[$row5['timing_project']]+=(($row5['timing_end']-$row5['timing_start'])+1)*(($row5['procent_harvest']/100)+1);
						}
				...
				$smarty->assign('hours_harvest',$hours_harvest);
		...
		if($_POST['submit']=='PDF') {
							...
							foreach($projects as $project) {
								...
								$rdata[]=array(
									...
									hm($hours_harvest[$project['project_id']]),
					...
					if($this->rewrite_params[3]) {
						...
						if($this->rewrite_params[4]=='getexcel') {
							...
							$tqables.= "<tr >
								...
								<td>NAME(S)</td>
								<td>HOURS (old)</td>
								<td>HOURS (Custom user)</td>
								<td>HOURS (Custom user) - Harvest</td>
							...
							$dates_array_h = explode(":", $data[5]);


                            $minutes=(((int)$dates_array[0]*60+(int)$dates_array[1])*60);
                            $minutes_h=(((int)$dates_array_h[0]*60+(int)$dates_array_h[1])*60);
							...
							$count1+=(((int)$dates_array_h[0].".".(sprintf("%02d",(round(($dates_array_h[1]*0.5)/0.3))))));
							$count2+=$minutes_h;
							$tqables.= "<tr><td>{$cclient}</td>
							<td>{$data[2]}</td>
							<td>{$data[3]}</td>
							<td>".(int)$dates_array[0].".".sprintf("%02d",(round(($dates_array[1]*0.5)/0.3)))."</td>
							<td>{$data[6]}</td>
							<td>".$data[4]."</td>
							<td>".(((int)$dates_array_h[0].".".sprintf("%02d",(round(($dates_array_h[1]*0.5)/0.3)))))."</td>
							<td>".hm($minutes_h)."</td>
							</tr>";
							...
							*/
							...
							$tqables.= '<tr><td></td><td></td><td></td><td></td><td></td><td></td><td>'.$count1.'</td><td>'.hm($count2).'</td></tr>';
...
$smarty->assign('hour',$hour);
		...
		function displayProject() {
			...
			if($id) {
				...
				while($row=mysql_fetch_assoc($res)) {
					$row['timing']=1;
					$q="SELECT * from timing,user where timing_project={$row['project_id']} and user_id=timing_user and delete_user=0";
					$res2=_sqlQuery($q);
					...
					if($this->rewrite_params[2]=='timing') {
						...
						$reztiming=($this->hm_harvest($timing_values[0]['timing_end']-$timing_values[0]['timing_start'])*($_SESSION['auth']['procent_harvest']/100)+$this->hm_harvest($timing_values[0]['timing_end']-$timing_values[0]['timing_start']));
...
mail('radu.munca@gmail.com','Timp schimbat  - eroare ','<b>User:</b> '.$_SESSION['auth']['user_name'].'<br> <b>ID:</b> '.$timing_values[0]['timing_project'].'<br> <b>Client:</b> '.$row['client_name'].' - '.$row['project_name'],$headers);
mail('officialbranding@gmail.com','Timp schimbat  - eroare ','<b>User:</b> '.$_SESSION['auth']['user_name'].'<br> <b>ID:</b> '.$timing_values[0]['timing_project'].'<br> <b>Client:</b> '.$row['client_name'].' - '.$row['project_name'],$headers);
            ...
            $headers .=  'From: ' . $from . "\n" . 'Reply-To: ' . $from . "\n";
					...
					if($this->rewrite_params[2]=='stop') {
						...
						$rez=($this->hm_harvest($timing_id_last[0]['timing_end']-$timing_id_last[0]['timing_start'])*($_SESSION['auth']['procent_harvest']/100)+$this->hm_harvest($timing_id_last[0]['timing_end']-$timing_id_last[0]['timing_start']));
						if($rez<0.01) { $rez=0.02; }

					//	if($row['project_client_harvest']=='19649383' ||  $row['project_client_harvest']=='19649410' ||  $row['project_client_harvest']=='19649473') { $taskid=8009975; } else { $taskid=12854003; }

						if($row['project_type']=='17') { 
                                                    $taskid=8009976;                                                     
                                                } elseif($row['project_type']=='13') { 
                                                    $taskid=8009975;                                                     
                                                } elseif($row['project_type']=='46') { 
                                                    $taskid=23189144;                                                     
                                                } elseif($row['project_type']=='47') { 
                                                    $taskid=23537203;                                                     
                                                } elseif($row['project_type']=='45') { 
                                                    $taskid=23189137;                                                     
                                                } elseif($row['project_type']=='44') { 
                                                    $taskid=23189145;                                                     
                                                } elseif($row['project_type']=='43') { 
                                                    $taskid=23189146;                                                     
                                                } elseif($row['project_type']=='42') { 
                                                    $taskid=23189136;                                                     
                                                } elseif($row['project_type']=='41') { 
                                                    $taskid=23189135;                                                 
                                                }
						if($row['project_client_harvest']=='33137135' ||  $row['project_client_harvest']=='33137145') { $taskid=8009975; }
...
mail('radu.munca@gmail.com','Oprire timp - eroare ','<b>User:</b> '.$_SESSION['auth']['user_name'].'<br> <b>ID:</b> '.$id.'<br> <b>Client:</b> '.$row['client_name'].' - '.$row['project_name'],$headers);
...
/* Display My Projects */
		function displayMyProjects() {
			...
			//get projects list
				...
				$projects=array();
			//deleteuser start
				$q="SELECT *  from project p,timing t where project_deleted=0 and project_id=timing_project and delete_user=0 and timing_user='{$this->uid}' {$q_tmp} group by project_id order by {$sort_name} {$sort} ,project_priority desc";
			//deleteuser end
				$res=_sqlQuery($q);
				while($row=mysql_fetch_assoc($res)) {
				...
				foreach($filters as $key=>$value) {
					...
					switch($key) {
						...
						case 'paid':
								...
								break;
						case 'projects_users':
								$qtmp_users="  and timing_user='{$value}' and delete_user=0";
								break;
					}
				}
...
*/
				...
				while($rower=mysql_fetch_assoc($rest)) {
					//deleteuser start
				$q="SELECT * from timing where delete_user=0 and timing_project={$rower['project_id']}";
					//deleteuser end
				$res2=_sqlQuery($q);
				...
				if($action=='edit') {
					...
					if($pid) {
						...
						//deleteuser start
						$q="SELECT *  from timing where timing_project=$pid and  delete_user=0 order by timing_user";
						//deleteuser end
				...
				if($_POST) {
						...
						$q="UPDATE project SET {$q_tmp} where project_id={$id}";
							...
							foreach($udel as $k=>$u) {
								...
								$q="UPDATE timing SET delete_user=1  where timing_user={$u['timing_user']} and timing_project={$id}";
								//deleteuser end
								_sqlQuery($q);
								//log delete user
		...
		function displayHome() {
			...
			if($action=='finished') {
				...
				*/
				...
				//deleteuser start
				$q="select * from project,ptype where project_type=ptype_id and project_status!=".PS_COMPLETED." and project_deleted=0 and project_id in (SELECT distinct(timing_project) from timing where delete_user=0 and timing_user={$this->uid})";
				//deleteuser end
