...
(function ($) {
	$.extend($.fn, {
		...
		rules: function (h, k) {
			...
			var g = $.validator.normalizeRules($.extend({},
			$.validator.metadataRules(f), $.validator.classRules(f), $.validator.attributeRules(f), $.validator.staticRules(f)), f);
			if (g.required) {
				var j = g.required;
				delete g.required;
				g = $.extend({
	...
	$.extend($.validator, {
		...
		prototype: {
			...
			elements: function () {
				var a = this,
				rulesCache = {};
				return $([]).add(this.currentForm.elements).filter(":input").not(":submit, :reset, :image, [disabled]").not(this.settings.ignore).filter(function () { ! this.name && a.settings.debug && window.console && console.error("%o has no name assigned", this);
...
(function ($) {
	...
	$.extend($.fn, {
		...
		triggerEvent: function (a, b) {
			return this.triggerHandler(a, [$.event.fix({
				type: a,
				target: b
			})])
		}
